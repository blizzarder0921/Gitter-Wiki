...
export function ModelSelector({
  ...
  // - Provider requires API key: must have client key
  // - Provider doesn't require API key (e.g. Ollama): must have explicit baseUrl
  ...
  // Filter models across all providers by search query
  ...
  const handleTestModel = useCallback(
    async (pid: ProviderId, mid: string) => {
      ...
      // 优先使用用户配置的 baseUrl，为空时回退到提供商默认值
      const baseUrl = providerConfig.baseUrl || providerConfig.defaultBaseUrl || '';
      ...
      try {
        ...
        if (data.success) {
          ...
          setTestStatus('error');
          setTestMessage(data.detail || t('settings.connectionFailed'));
        }
      } catch {
        setTestStatus('error');
