...
export function ProviderConfigPanel({
  ...
  // 判断是否已配置：使用本地输入的 apiKey 或全局配置中的 apiKey
  const isConfigured = !!(apiKey || providerConfig?.apiKey);
      ...
      <div className="space-y-3">
        ...
        {!isConfigured && (
          <div className="rounded-lg border border-dashed border-border/50 p-6 text-center text-sm text-muted-foreground">
            {t('settings.configureApiKeyToSeeModels') || '请先配置 API Key 以查看模型列表'}
          </div>
        )}
      </div>
