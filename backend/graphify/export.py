...
def to_html(
    ...
    if G.number_of_nodes() > limit:
        if node_limit is not None:
            ...
            # 为没有 LLM 标签的社区生成基于内容的描述性标签
            # community_labels 中可能包含 "Community X" 格式的默认占位标签（由 watch.py 填充），
            # 需要过滤掉这些占位标签，用基于内容的标签替代
            import re as _re
            _community_placeholder = _re.compile(r"^Community \d+$")
            auto_labels: dict[int, str] = {}
            for cid, members in communities.items():
                existing_label = (community_labels or {}).get(cid, "")
                if existing_label and not _community_placeholder.match(existing_label):
                    auto_labels[cid] = existing_label
                    continue
                # 收集社区成员的文件类型和源文件路径
                file_types: list[str] = []
                ...
                for nid in members:
                    ...
                    if sf:
                        ...
                        if len(parts) > 1:
                            source_dirs.append(parts[0])
                        else:
                            source_dirs.append(sf)
                    # 收集度数最高的节点标签
                    lbl = ndata.get("label", "")
                    if lbl:
                        top_nodes.append((degree.get(nid, 0), lbl))
                # 构建标签：优先用模块目录名，其次用文件类型，最后用最高度节点
                ft_counter = _Counter(file_types)
                dir_counter = _Counter(source_dirs)
                top_nodes.sort(key=lambda x: -x[0])
                parts: list[str] = []
                if dir_counter:
                    top_dir = dir_counter.most_common(1)[0][0]
                    parts.append(top_dir)
                elif ft_counter:
                    top_ft = ft_counter.most_common(1)[0][0]
                    ft_display = {"code": "Code", "document": "Docs", "rationale": "Logic"}.get(top_ft, top_ft.title())
                    parts.append(ft_display)
                if top_nodes:
                    parts.append(top_nodes[0][1])
                if parts:
                    auto_labels[cid] = " · ".join(parts)
                else:
                    auto_labels[cid] = f"Community {cid}"

            meta = _nx.Graph()
            for cid, members in communities.items():
                lbl = auto_labels.get(cid, f"Community {cid}")
                meta.add_node(str(cid), label=lbl)
            ...
            to_html(meta, meta_communities, output_path,
                    community_labels=auto_labels, member_counts=mc)
